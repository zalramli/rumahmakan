                                ...
                                foreach ($query as $data) :
                                    ...
                                    <div class="col-md-4 mb-3">
                                        <div style="padding:0" class="card">
                                            <div class="card-body">
                                                    <img width="170" class="mb-2 rounded" height="100" src="<?php echo "assets/img/".$data['photo']; ?>">
                                                <p style="margin:0" class="text-center"><?php echo $data['nama_barang'] ?></p>
                                                <p style="margin:0" class="text-center"><?php echo rupiah($data['harga_jual']) ?></p>
                                                <div class="text-center">
                                                <a onclick="tambah('<?php echo $kode ?>','<?php echo $nama ?>','<?php echo $harga_jual  ?>')" class="btn btn-sm btn-primary text-white mt-2">Pilih Menu</a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <form method="post" id="myform" action="">
                                <table class="table table-sm" width="100%">
                                    ...
                                    <div class="row mb-3">
                                        ...
                                        <div class="col-md-4">
                                            <input type="text" class="form-control form-control-sm rupiah text-right" name="bayar" id="bayar" onkeyup="kembalian()" required>
                                        </div>
                                        <div class="col-md-4">
                                            <input type="text" class="form-control form-control-sm rupiah text-right" name="kembali" id="kembali" value=" " readonly>
                                        </div>
                                    </div>
                                    <tbody>
                                    <tr>
                                        <td><button type="submit" name="simpan" class="btn btn-sm btn-primary">Simpan Data</button></td>
                                    </tr>
                                    </tbody>
                                </table>
                            </form>
    ...
    function tambah(kode,nama,harga){
        ...
        $('#detail_list').append(`
            ...
            <tr id="row` + count + `" class="kelas_row">
                <td>
                    ` + nama + `
                    <input type="hidden" name="kode_barang[]" class="form-control form-control-sm" id="kode_barang` + count + `" value="` + kode + `">
                </td>
                <td>
                    <input type="text" name="qty[]" class="form-control form-control-sm qty" id="qty` + count + `" value="1" required>
                </td>
                <td>
                    <input type="text" name="harga[]" class="form-control form-control-sm rupiah text-right" id="harga` + count + `" placeholder="Harga Tindakan BP" required value="` + harga + `" readonly>
                </td>
                <td>  
                    <input type="text" class="form-control form-control-sm rupiah text-right" id="sub_total` + count + `" readonly required value="` + harga + `"></td>
                <td>
